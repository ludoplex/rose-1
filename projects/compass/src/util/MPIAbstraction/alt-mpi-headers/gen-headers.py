...
def open_file(fn, mode = 'r'):
    ...
    if fn == '-':
        ...
        fp = open (fn, mode) or abort(f"Can't open file, '{fn}'.")
    return fp
...
mpi_typedef_restr = f"typedef (.*) ({mpi_typename_restr});"
...
def match_mpi_typedef(line):
    ...
    if m := mpi_typedef_matcher.match(line):
...
mpi_typed_def3_restr = (
    f"# *define +({mpi_id_restr}" + ")\s+\(\s*(.*)\s*\)\s*([0-9]+)"
)
...
def rewrite_mpi_header(fp_in, fp_out, new_defs):
    ...
    for line in fp_in.xreadlines ():
        ...
        if m := match_mpi_def(line):
            new_externs[m['id']] = "extern const int %s; /* == %s @MOD@ */\n" \
                                   % (m['id'], m['val'])
            write_line(fp_out, f"/* Replacing #define {m['id']} */")
            new_defs[m['id']] = "const int %s = %s;\n" % (m['id'], m['val'])
            continue

        if m := match_mpi_typed_def(line):
...
opts_flag_str = f"--({opts_re_str})"
opts_flag_matcher = re.compile(f"{opts_flag_str}$")
opts_matcher = re.compile(f"{opts_flag_str}=(.*)$")
i = 1
while i < len (sys.argv):
    ...
    if m := opts_flag_matcher.match(arg):
        usage(f"Unrecognized option, '{m.group(0)}'.")
...
filenames = {
    ...
    'mpi-header-in': f'{sys.argv[i]}/include/mpi.h',
    'mpi-header-out': opts['out-incs'] + '/mpi.h',
    'mpi-header-defs-in': f'{sys.argv[i]}/include/mpidefs.h',
    'mpi-header-defs-out': opts['out-incs'] + '/mpidefs.h',
    'mpi-header-fortdefs-in': f'{sys.argv[i]}/include/mpi_fortdefs.h',
    'mpi-header-fortdefs-out': opts['out-incs'] + '/mpi_fortdefs.h',
    'mpi-header-io-in': f'{sys.argv[i]}/include/mpio.h',
    'mpi-header-io-out': opts['out-incs'] + '/mpio.h',
    'mpi-header-errno-in': f'{sys.argv[i]}/include/mpi_errno.h',
...
for dn in ['out-incs', 'out-srcs']:
    mkdir_p(filenames[dn]) or abort(
        f"Can't create output directory, '{filenames[dn]}'."
    )
...
for (fn_in, fn_out) in rewrite_file_pairs:
    ...
    # instead
    write_line(
        fp_out, f'// "{filenames[fn_out]}, derived from {filenames[fn_in]}."'
    )
